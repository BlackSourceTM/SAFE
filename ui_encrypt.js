...
var scopeBlock=ce("div","safe-field");scopeBlock.style.display="none";
var scopeLab=ce("label","safe-field__label");scopeLab.textContent="Access lock (paid plans only)";
var scopeWrap=ce("div","safe-radio-group");
var optOwner=ce("label","safe-radio");
var optOwnerInput=ce("input");optOwnerInput.type="radio";optOwnerInput.name="safe-enc-scope";optOwnerInput.value="owner";
var optOwnerText=d.createTextNode("Only this Telegram account (owner lock)");
optOwner.appendChild(optOwnerInput);optOwner.appendChild(optOwnerText);
var optAny=ce("label","safe-radio");
var optAnyInput=ce("input");optAnyInput.type="radio";optAnyInput.name="safe-enc-scope";optAnyInput.value="any";optAnyInput.checked=true;
var optAnyText=d.createTextNode("Any account with the password");
optAny.appendChild(optAnyInput);optAny.appendChild(optAnyText);
scopeWrap.appendChild(optOwner);scopeWrap.appendChild(optAny);
scopeBlock.appendChild(scopeLab);scopeBlock.appendChild(scopeWrap);
...
var currentStep=1;var lastPrecheck=null;
...
function evalStep1(){var fileOk=fInp.files&&fInp.files[0],pwd=pInp.value||"",res=strength(pwd);updateRules(rules,res);updateMeter(meter,res.level);if(!pwd)updateMeter(meter,0);btnNext.disabled=!(fileOk&&res.level>=4&&res.rules.latin)}
function evalStep2(){var a=pInp.value||"",b=cInp.value||"";if(!b){setInputState(cInp,"");cErr.textContent="";btnStart.disabled=true;return}if(a===b){setInputState(cInp,"ok");cErr.textContent="";btnStart.disabled=false}else{setInputState(cInp,"error");cErr.textContent="Passwords do not match.";btnStart.disabled=true}}
function gotoStep1(){currentStep=1;cBlock.style.display="none";scopeBlock.style.display="none";rules.style.display="block";meterWrap.style.display="block";btnBack.style.display="none";btnNext.style.display="inline-flex";btnStart.style.display="none";btnStart.disabled=true;pInp.readOnly=false;btnGen.style.display="inline-flex";btnEye.style.display="inline-flex";setErr("");setStatus("Ready");evalStep1()}
function gotoStep2(){currentStep=2;cBlock.style.display="block";rules.style.display="none";meterWrap.style.display="none";btnBack.style.display="inline-flex";btnNext.style.display="none";btnStart.style.display="inline-flex";btnStart.disabled=true;pInp.readOnly=true;btnGen.style.display="none";btnEye.style.display="none";cInp.value="";setInputState(cInp,"");cErr.textContent="";scopeBlock.style.display="none";setErr("");setStatus("Confirm your password.")}
function gotoStep3(planId){if(planId==="free"){scopeBlock.style.display="none";return}currentStep=3;cBlock.style.display="none";rules.style.display="none";meterWrap.style.display="none";scopeBlock.style.display="block";btnBack.style.display="inline-flex";btnNext.style.display="none";btnStart.style.display="inline-flex";btnStart.disabled=false;pInp.readOnly=true;btnGen.style.display="none";btnEye.style.display="none";setErr("");setStatus("Choose access lock for this file.")}
function maskKey(p){p=p||"";if(p.length<=4)return p;return p.slice(0,2)+"****"+p.slice(-2)}
function showTermsAndStart(flow){var old=q(".safe-popup-backdrop");if(old&&old.parentNode)old.parentNode.removeChild(old);var bg=ce("div","safe-popup-backdrop"),pop=ce("div","safe-popup"),t=ce("div","safe-popup__title"),txt=ce("div","safe-popup__text"),acts=ce("div","safe-popup__actions"),bTerms=ce("button","btn btn--ghost btn--sm"),bOk=ce("button","btn btn--primary btn--sm");t.textContent="SAFE – terms of use";txt.textContent="By starting encryption you confirm that you have read and accepted the SAFE rules and terms of use.";bTerms.type="button";bOk.type="button";bTerms.textContent="Terms of use";bOk.textContent="OK";acts.appendChild(bTerms);acts.appendChild(bOk);pop.appendChild(t);pop.appendChild(txt);pop.appendChild(acts);bg.appendChild(pop);d.body.appendChild(bg);bOk.onclick=function(){if(bg.parentNode)bg.parentNode.removeChild(bg);flow()};bTerms.onclick=function(){if(bg.parentNode)bg.parentNode.removeChild(bg);close();var nav=q('.nav-item[data-section="support"]');if(nav)nav.click()}}
function doPrecheckAndStart(){
var file=fInp.files&&fInp.files[0],pwd=pInp.value||"";if(!file||!pwd)return;
setErr("");setStatus("Checking your SAFE account...");
...
if(!tg||!tg.id){var msg1="Cannot detect Telegram WebApp user. Please open SAFE from the official bot.";setStatus("Cannot detect account.");setErr(msg1);notify(msg1,"error");return}
var uid=tg.id;
function maskKeySent(p){return maskKey(p)}
var payload={user_id:uid,file_name:file.name,file_size_bytes:file.size,mime_type:file.type||"",password_mask:maskKeySent(pwd)};
fetch("https://safe-bot-worker.alirahimikiasari.workers.dev/api/encrypt/precheck",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)}).then(function(res){return res.json().catch(function(){return null})}).then(function(data){
if(!data||!data.ok){var msg2=(data&&data.message)||"Check failed";setErr(msg2);setStatus(msg2);notify(msg2,"error");return}
lastPrecheck=data;var planId=data.plan||"free";var scopeVal="any";if(planId!=="free"){if(optOwnerInput.checked)scopeVal="owner";else scopeVal="any"}setStatus("Encryption is starting...");notify("Account OK. Encryption started.","success");
if(w.dispatchEvent&&w.CustomEvent){var ev=new CustomEvent("safe:start-encryption",{detail:{file:file,password:pwd,meta:data,scope:scopeVal}});w.dispatchEvent(ev)}
close()
}).catch(function(){
var msg3="Cannot contact SAFE backend. Please try again.";setErr(msg3);setStatus(msg3);notify(msg3,"error")
})
}
fInp.onchange=evalStep1;pInp.oninput=evalStep1;
btnGen.onclick=function(){var c="ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz23456789!@#$%^&*()-_=+[]{}",out="",i;for(i=0;i<16;i++)out+=c.charAt(Math.floor(Math.random()*c.length));pInp.value=out;evalStep1()};
...
btnBack.onclick=function(){if(currentStep===2){gotoStep1()}else if(currentStep===3){gotoStep2()}};
btnNext.onclick=function(){if(btnNext.disabled)return;gotoStep2()};
btnStart.onclick=function(){
if(currentStep===2){
var file=fInp.files&&fInp.files[0],pwd=pInp.value||"";if(!file||!pwd)return;
showTermsAndStart(function(){
var planId="free";if(lastPrecheck&&lastPrecheck.plan)planId=lastPrecheck.plan;
if(planId==="free"){doPrecheckAndStart()}else{gotoStep3(planId)}
})
}else if(currentStep===3){
showTermsAndStart(function(){doPrecheckAndStart()})
}};
gotoStep1();root.classList.add("safe-modal-root--visible")}
function bind(){var b=q("#btn-encrypt-new");if(!b)return;var clone=b.cloneNode(true);clone.id=b.id;b.parentNode.replaceChild(clone,b);clone.addEventListener("click",openEncrypt)}
