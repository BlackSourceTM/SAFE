...
function saveBlobInteractive(name,blob){
  return new Promise(function(resolve){
    var g=w||window;var doc=g.document;var URLg=g.URL||window.URL;
    if(g.showSaveFilePicker){
      g.showSaveFilePicker({suggestedName:name,types:[{description:"SAFE file",accept:{"application/octet-stream":[".SAFE",".safe",".*"]}}]}).then(function(handle){
        return handle.createWritable()
      }).then(function(writer){
        return writer.write(blob).then(function(){return writer.close()})
      }).then(function(){
        resolve({ok:true,method:"picker",aborted:false,error:null})
      }).catch(function(err){
        if(err&&err.name==="AbortError")resolve({ok:false,method:"picker",aborted:true,error:err});
        else resolve({ok:false,method:"picker",aborted:false,error:err})
      })
    }else if(doc&&URLg){
      var url=URLg.createObjectURL(blob);
      var a=doc.createElement("a");
      a.href=url;a.download=name;doc.body.appendChild(a);a.click();
      setTimeout(function(){try{doc.body.removeChild(a)}catch(e){}try{URLg.revokeObjectURL(url)}catch(e){}},0);
      resolve({ok:true,method:"download",aborted:false,error:null})
    }else{
      resolve({ok:false,method:"none",aborted:false,error:new Error("NO_SAVE_METHOD")})
    }
  })
}
function encryptAndSave(file,password,onProgress){
  if(!file||!password)return Promise.reject(new Error("INVALID_INPUT"));
  return encryptFile(file,password,function(p,s){onProgress&&onProgress(p,s)}).then(function(res){
    onProgress&&onProgress(.9,"saving");
    return saveBlobInteractive(res.name,res.blob).then(function(saveRes){
      onProgress&&onProgress(1,"done");
      return{blob:res.blob,name:res.name,meta:res.meta,save:saveRes}
    })
  })
}
function decryptAndSave(file,password,onProgress){
  if(!file||!password)return Promise.reject(new Error("INVALID_INPUT"));
  return decryptFile(file,password,function(p,s){onProgress&&onProgress(p,s)}).then(function(res){
    onProgress&&onProgress(.9,"saving");
    return saveBlobInteractive(res.name,res.blob).then(function(saveRes){
      onProgress&&onProgress(1,"done");
      return{blob:res.blob,name:res.name,meta:res.meta,save:saveRes}
...
w.SAFE_CRYPTO={encryptFile:encryptFile,decryptFile:decryptFile,encryptAndSave:encryptAndSave,decryptAndSave:decryptAndSave};
})(window);
