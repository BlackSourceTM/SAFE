...
function canUseFSA(){return typeof w!=="undefined"&&"showSaveFilePicker"in w}
function safeSuggestedName(name,forceSafeExt){
 var base=name||"file";
 base=base.replace(/[/\\<>:"|?*\x00-\x1F]/g,"_");
 if(forceSafeExt)base=base.replace(/\.[^.]+$/,"")+".safe";
 return base;
}
async function saveBlobWithPicker(blob,defaultName,forceSafeExt){
 if(!canUseFSA())throw new Error("NO_FSA");
 var name=safeSuggestedName(defaultName,forceSafeExt);
 var m=name.match(/\.([A-Za-z0-9]+)$/);
 var ext=m?"."+m[1].toLowerCase():".bin";
 var opts={suggestedName:name,types:[{description:"File",accept:{"application/octet-stream":[ext]}}]};
 var handle=await w.showSaveFilePicker(opts);
 var wr=await handle.createWritable();
 await wr.write(blob);
 await wr.close();
 return name;
}
function downloadViaLink(blob,defaultName,forceSafeExt){
 var name=safeSuggestedName(defaultName,forceSafeExt);
...
async function saveEncryptedResult(result){
 var blob=result.blob;
 var name=result.name||"file.safe";
 var finalName=name;
 try{finalName=await saveBlobWithPicker(blob,name,true)}catch(e){downloadViaLink(blob,name,true)}
 if(w.SAFE_UI_CORE&&w.SAFE_UI_CORE.showToast)w.SAFE_UI_CORE.showToast("Encrypted file ready: "+finalName,"success");
}
async function saveDecryptedResult(result){
 var blob=result.blob;
 var name=result.name||"file";
 var finalName=name;
 try{finalName=await saveBlobWithPicker(blob,name,false)}catch(e){downloadViaLink(blob,name,false)}
 if(w.SAFE_UI_CORE&&w.SAFE_UI_CORE.showToast)w.SAFE_UI_CORE.showToast("Decrypted file ready: "+finalName,"success");
}
function handleEncEvent(ev){
 var detail=ev.detail||{};
 var file=detail.file;
 var pwd=detail.password||detail.pass||"";
 if(!file||!pwd){if(w.SAFE_UI_CORE&&w.SAFE_UI_CORE.showToast)w.SAFE_UI_CORE.showToast("Encryption failed: missing file or password.","error");return}
 encryptFile(file,pwd,function(p,phase){}).then(function(res){return saveEncryptedResult(res)}).catch(function(err){if(w.SAFE_UI_CORE&&w.SAFE_UI_CORE.showToast)w.SAFE_UI_CORE.showToast("Encryption error: "+(err&&err.message||String(err)),"error")});
}
function handleDecEvent(ev){
 var detail=ev.detail||{};
 var file=detail.file;
 var pwd=detail.password||detail.pass||"";
 if(!file||!pwd){if(w.SAFE_UI_CORE&&w.SAFE_UI_CORE.showToast)w.SAFE_UI_CORE.showToast("Decryption failed: missing file or password.","error");return}
 decryptFile(file,pwd,function(p,phase){}).then(function(res){return saveDecryptedResult(res)}).catch(function(err){if(w.SAFE_UI_CORE&&w.SAFE_UI_CORE.showToast)w.SAFE_UI_CORE.showToast("Decryption error: "+(err&&err.message||String(err)),"error")});
}
